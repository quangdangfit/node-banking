...
exports.list = () => kn('users').select('username', 'first_name', 'last_name', 'email', 'phone');
...
exports.add = input => {
  ...
  return kn('users').insert(input).returning('uid').returning('uid')
};
...
exports.login = (input) => {
  ...
  return kn('users').select('uid').where({
